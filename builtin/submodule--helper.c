...
static void print_submodule_summary(struct summary_cb *info, int errmsg,
				int total_commits, int missing_src, int missing_dst,
				const char *displaypath, int is_sm_git_dir,
				struct module_cb *p)
{
	if (p->status == 'T') {
				       ...
				       struct module_cb *p)
{
	...
	if (!info->cached && oideq(&p->oid_dst, &null_oid)) {
		if (S_ISGITLINK(p->mod_dst)) {
			...
			argv_array_pushl(&cp_rev_parse.args, "rev-parse", "HEAD", NULL);
...
static int module_summary(int argc, const char **argv, const char *prefix)
{
	...
	struct strbuf sb = STRBUF_INIT;
	enum diff_cmd_info diff_cmd = DIFF_INDEX;
	int ret;
