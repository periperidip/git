...
static void print_submodule_summary(struct summary_cb *info, int errmsg,
				      ...
				      int is_sm_git_dir, const char *src_oid_hash,
				      const char *dst_oid_hash,
				      struct module_cb *p)
{
	if (p->status == 'T') {
				       ...
				       struct module_cb *p)
{
	...
	print_submodule_summary(info, errmsg, total_commits,
				displaypath, is_sm_git_dir,
				src_oid_hash, dst_oid_hash,
				p);
		...
		if (info->for_status && p->status != 'A') {
			...
			if(sub)
				config_key = xstrfmt("submodule.%s.ignore",
					     sub->name);
			if (sub && !git_config_get_string_const(config_key, &value)) {
				ignore_config = value;
				free(config_key);
			}
			...
			if (sub && !strcmp(ignore_config, "all"))
				continue;
		}
