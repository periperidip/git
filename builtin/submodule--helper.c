	...
	if (p->status == 'T') {
		if (S_ISGITLINK(p->mod_dst))
			printf(_("* %s %s(blob)->%s(submodule)"),
				 displaypath, src_committish, 7), dst_committish);
		else
...
static void generate_submodule_summary(struct summary_cb *info,
				       struct module_cb *p)
{
	char *displaypath, *src_oid_hash = NULL, *dst_oid_hash = NULL;
	int errmsg = 0;
	int total_commits = -1;
	...
	if (!S_ISGITLINK(p->mod_src) || !src_oid_hash)
		src_oid_hash = xstrndup(oid_to_hex(&p->oid_src), 7);

	if (S_ISGITLINK(p->mod_src) && !(src_oid_hash = verify_submodule_committish(p->sm_path,
							oid_to_hex(&p->oid_src))))
		src_oid_hash = NULL;
	...
	if (!S_ISGITLINK(p->mod_dst) || !dst_oid_hash)
		dst_oid_hash = xstrndup(oid_to_hex(&p->oid_dst), 7);

	if (S_ISGITLINK(p->mod_dst) && !(dst_oid_hash = verify_submodule_committish(p->sm_path,
							oid_to_hex(&p->oid_dst))))
		dst_oid_hash = NULL;
